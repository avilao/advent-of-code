...
class IntCode {
   ...
   decodeInstruction() {
     ...
     return {
       code,
       operator: utils.getNumberAtPosition(code, 1) + utils.getNumberAtPosition(code, 2) * 10,
       params: [
         {
     ...
     switch (instruction.operator) {
       case 1: 
         this.write(
           ...
           this.getReadValueFromParam(instruction.params[0]) + this.getReadValueFromParam(instruction.params[1])
         );
         this.position += 4;
         break;
       case 2:
       ...
       case 4:
         ...
         break;
       case 5:
         this.position = this.getReadValueFromParam(instruction.params[0]) !== 0 ? this.getReadValueFromParam(instruction.params[1]) : this.position + 3;
         break;
       case 6: 
         this.position = this.getReadValueFromParam(instruction.params[0]) === 0 ? this.getReadValueFromParam(instruction.params[1]) : this.position + 3;
         break;
       case 7:
         this.write(
           instruction.params[2],
           this.getReadValueFromParam(instruction.params[0]) < this.getReadValueFromParam(instruction.params[1]) ? 1 : 0
         );
         this.position += 4;
         break;
       case 8:
